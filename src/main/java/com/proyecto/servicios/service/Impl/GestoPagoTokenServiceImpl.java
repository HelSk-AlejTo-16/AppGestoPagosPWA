...
public class GestoPagoTokenServiceImpl implements GestoPagoTokenService {
    ...
    public Optional<GestoPagoToken> obtenerTokenActivo(Integer idDistribuidor, String codigoDispositivo) {
        ...
        @Override
    public String obtenerBearerToken(Integer idDistribuidor, String codigoDispositivo) {
        Optional<GestoPagoToken> enBd = obtenerTokenActivo(idDistribuidor, codigoDispositivo);
        if (enBd.isPresent()) {
            return "Bearer " + enBd.get().getToken();
        }
        if (tokenRespaldo != null && !tokenRespaldo.isBlank()) {
            log.warn("No hay token en BD; se usa el token de respaldo de la configuración");
            return "Bearer " + tokenRespaldo;
        }
        throw new IllegalStateException("No hay token disponible en BD ni en la configuración.");
    }
}
